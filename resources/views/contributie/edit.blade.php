<x-layout>
    <button class="terug"><a href="/contributie">Terug</a></button><br><br>
    {{-- @php
        dd($contributie);
    @endphp --}}
    <p>Bewerk contributie:</p>
    <div>
        <form action="/contributie/{{$contributie->id}}" method="POST">
            @csrf
            @method('PUT')
            <label for="leeftijd">Leeftijd:</label>
            <input type="text" name="leeftijd" id="leeftijd" value="{{ $contributie->leeftijd }}" required ><br>
            ...
            <input type="number" step="0.01" name="bedrag" id="bedrag" value="{{ $contributie->bedrag }}" required><br><br>
